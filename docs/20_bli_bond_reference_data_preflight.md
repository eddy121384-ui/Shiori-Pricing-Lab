...
| `coupon` | required | **Required** | `float` | Coupon *rate* (per Annex B "coupon"). Decimal form, consistent with `docs/18`'s percent-vs-decimal rule for FTP rates — this doc does not re-litigate that; it simply requires decimal, not percent, matching every other rate field in this repo. |
...
| `first_coupon_date` | required | Optional for MVP | `str \| None` (`YYYY-MM-DD`) | Annex B marks it required and flags "irregular period → must enter cash flow generation, cannot be ignored" (§B.5 validation note). For a *reference-data schema* (not a cash-flow generator), recording the date is cheap and safe; treating it as MVP-optional here only means a regular-coupon bond fixture may omit it, not that irregular-period cash-flow generation is in scope — that remains pricing-engine work (§10 defers it explicitly). |
| `last_coupon_date` | required | Optional for MVP | `str \| None` (`YYYY-MM-DD`) | Same reasoning as `first_coupon_date`. |
...
coupon must be a finite number (reuse _require_finite_number);
  sign is not force-checked here -- a zero-coupon bond is a legitimate
  plain-vanilla instrument, so no positivity requirement unless a future
  slice decides zero-coupon is out of MVP scope
...
first_coupon_date, last_coupon_date, if present, must be strict YYYY-MM-DD
...
- **Irregular first/last coupon period cash-flow generation** (Annex B
  ...
  slice, not this preflight or its recommended next slice.
...
- Provides tests for:
  ...
  - missing required fields (§4) are rejected;
