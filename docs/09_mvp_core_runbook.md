...
- **Curve-rate-basis blocker (Codex P2 review of PR #71):**
  `BLICurvePoint` has no `rate_basis`/`curve_rate_basis`/`zero_vs_par`/
  `compounding` field, so nothing today confirms `rate` is already a
  continuously-compounded zero rate — `curve_purpose` says what a curve
  is *used for*, not what basis its rates are quoted in. **An
  interpolated-zero-rate helper is blocked** until a future PR resolves
  this via an explicit basis field/contract, a documented and audited
  upstream normalization guarantee, or a separate reviewed
  par-to-zero/source-to-zero conversion slice — never by silently
  inferring the basis from `curve_purpose`/`curve_name`/`curve_id`/
  `source_system`. A curve-purpose selector is still probably safe as a
  purely structural filter, but must not claim its output rates are
  already zero rates.
- **Discount-factor boundary decided conservatively, now doubly
  blocked:** a discount-factor helper stays out of scope for the
  foreseeable next several slices — behind both the curve-purpose
  selector and the interpolated-zero-rate helper, itself blocked by the
  curve-rate-basis gap above. If that gap is resolved, the formula to
  review is already unambiguous (`DF = exp(-zero_rate × T)`) — kept
  separate to avoid conflating it with `irs_engine.py`'s incompatible
  simple-compounding formula in the same change.
- **Next implementation slice chosen (unchanged): a `BLICurvePoint`
  tenor-to-year-fraction parser only** (`pricing/bli_curve_tenor.py`,
  suggested branch `claude/bli-curve-tenor-parser`) — it never reads or
  interprets `rate`, so it is entirely unaffected by the curve-rate-basis
  blocker. Zero design
...
### BLI curve-purpose selector landed (pure structural filter)

- `docs/27`'s next dependency-sequence slice is implemented in
  `src/shiori_pricing_lab/pricing/bli_curve_selector.py` (new module):
  `select_curve_points_by_purpose(curve_points, *, currency,
  curve_purpose) -> tuple[BLICurvePoint, ...]` narrows a collection of
  `BLICurvePoint` rows to exactly the ones matching
  `point.currency == currency` and `point.curve_purpose ==
  curve_purpose`, preserving input order. Accepts either an enum member
  or a raw string for `currency`/`curve_purpose` via the project's
  existing `coerce_enum` pattern. Raises `TypeError` for a
  non-`BLICurvePoint` element, and `ValueError` (naming the requested
  currency and purpose) for an empty input or no matching rows — never
  silently falls back to a different currency or purpose.
- **This selector is purely structural.** It does not parse tenor, does
  ...
  rate value. It does not claim the rates it returns are zero rates,
  and it **does not unblock `docs/27` §6.1's curve-rate-basis question**
  — that remains fully open. This slice does not implement
  interpolation and does not implement a discount factor.
- `pricing/bli_pricing_engine.py`, `pricing/bli_valuation_time.py`, and
  `pricing/bli_curve_tenor.py` are all unmodified; `price_bli_mvp` still
  returns its existing deterministic `PricingResult(status=FAILED,
  errors=[PricingErrorCode.UNSUPPORTED_PRODUCT])` for every valid
  bundle.
- Tests: `tests/test_bli_curve_selector.py` (selecting each of the three
  MVP-required curve purposes from
  `SYNTHETIC_BLI_MARKET_DATA_SNAPSHOT.curve_points` with no new fixture
  content; input-order preservation including a reversed-input and a
  non-tenor-sorted case; string-vs-enum coercion; missing-purpose and
  missing-currency cases with the error naming the missing value;
  empty-input and non-`BLICurvePoint`-element rejection; invalid
  currency/purpose string rejection; and module-boundary checks that
  the module never imports `bli_curve_tenor`, never reads a curve
  point's rate value, never calls wall-clock time, and defines no
  interpolation/discount-factor/zero-rate/par-rate/forward-price/
  Black-76/PV-shaped names). `python -m pytest -q` → 732 passed; `ruff
  check src/shiori_pricing_lab tests` → only the same 2 pre-existing,
  unrelated `products/bond_option.py` `E501` findings remain.
- **Issue #38 remains open.**
...
### BLI curve rate-basis contract landed

- `docs/28`'s recommended next slice (Path A) is implemented in
  `src/shiori_pricing_lab/data/bli_snapshot.py`: a new
  `BLICurveRateBasis(StrEnum)` (`CONTINUOUS_ZERO_RATE` /
  `SIMPLE_ZERO_RATE` / `PAR_RATE` / `SWAP_RATE` / `BOND_YIELD` /
  `FUNDING_RATE` / `OTHER`) and a new **required**
  `BLICurvePoint.rate_basis: BLICurveRateBasis` field, validated via
  `coerce_enum` in `__post_init__` (mirroring `curve_purpose`'s own
  coercion pattern).
- **No default is provided.** A `BLICurvePoint` cannot be constructed
  without explicitly stating `rate_basis` — omitting it raises
  `TypeError` (the dataclass's own missing-required-field error), and
  there is no "if missing, assume zero rate" fallback anywhere.
  ...
  what a row's `rate` means.
- **All existing fixture/test curve points now state basis explicitly.**
  Every row in `data/bli_snapshot_fixtures.py`'s
  `SYNTHETIC_BLI_MARKET_DATA_SNAPSHOT` is marked
  `rate_basis=BLICurveRateBasis.CONTINUOUS_ZERO_RATE` — a reviewed
  synthetic-fixture assumption for the current BLI path only, stated
  explicitly, never inferred from `source_system`/`curve_id`/
  `curve_name`/`curve_purpose`/tenor. Every `_curve_point()` test helper
  (`tests/test_bli_market_data_snapshot.py`,
  `tests/test_bli_mvp_input_bundle.py`,
  `tests/test_bli_mvp_input_bundle_builder.py`) and the two hand-built
  `BLICurvePoint` instances in `tests/test_bli_curve_selector.py` were
  updated the same way.
- **This does not implement interpolation, does not implement discount
  factors, and does not implement par-to-zero conversion.** It only
  ...
  remain unmodified — the selector still filters only by
  `(currency, curve_purpose)` and never reads `rate_basis` or `rate`.
  **This does not make `price_bli_mvp` price anything** — it still
  returns its existing deterministic `PricingResult(status=FAILED,
  errors=[PricingErrorCode.UNSUPPORTED_PRODUCT])` for every valid
  bundle, unchanged.
- Tests: happy-path basis check; raw-string coercion; invalid and blank
  `rate_basis` rejection (both naming `rate_basis`); a
  missing-required-field construction test that deliberately bypasses
  the test helper's default; a check that every fixture curve point
  states `CONTINUOUS_ZERO_RATE`; and a module-boundary check that
  `data/bli_snapshot.py` still defines no
  interpolation/discount-factor/zero-rate/par-rate/forward-price/
  Black-76/PV-shaped names. `python -m pytest -q` → 740 passed; `ruff
  check src/shiori_pricing_lab tests` → only the same 2 pre-existing,
  unrelated `products/bond_option.py` `E501` findings remain.
- **Issue #38 remains open.**

### BLI continuous zero-rate curve node preparation helper landed

- **This is a real implementation slice, not a preflight.** Continuing
  the dependency chain built by PR #72 (tenor parser), PR #73
  (curve-purpose selector), and PR #75 (required `rate_basis`
  contract): `src/shiori_pricing_lab/pricing/bli_zero_curve_nodes.py`
  (new module) adds `build_continuous_zero_curve_nodes(curve_points) ->
  tuple[BLIContinuousZeroCurveNode, ...]`, converting already-selected
  `BLICurvePoint` rows into sorted, validated continuous-zero-rate
  curve nodes — the calculation-preparation step between "pick one
  curve's rows" (`select_curve_points_by_purpose`) and any future
  interpolation helper.
- `BLIContinuousZeroCurveNode` (frozen dataclass): `year_fraction`,
  `zero_rate`, `source_tenor`, `source_curve_id`.
- **Rate-basis gate:** every input row must already have `rate_basis is
  BLICurveRateBasis.CONTINUOUS_ZERO_RATE` — any other basis raises
  `ValueError` naming the offending tenor, curve_id, actual basis, and
  expected `CONTINUOUS_ZERO_RATE`. Nothing is converted, and nothing
  infers continuous-zero status from `curve_purpose`/`curve_id`/
  `curve_name`/`source_system`/tenor.
- **Tenor parsing** delegates to the existing
  `pricing/bli_curve_tenor.py::tenor_to_year_fraction` — no other tenor
  parser is used or defined.
- Rejects empty input and any non-`BLICurvePoint` element, rejects
  duplicate parsed `year_fraction` values (e.g. `"12M"`/`"1Y"` both
  mapping to `1.0`), and returns nodes **sorted by `year_fraction`
  ascending regardless of input order**. Signed rates are preserved
  as-is.
- **This helper does not select curve purpose or currency, does not
  interpolate, does not extrapolate, does not compute a discount
  factor, forward clean price, or PV, and is not wired into
  `price_bli_mvp`.** `pricing/bli_pricing_engine.py`,
  ...
  still returns its existing deterministic
  `PricingResult(status=FAILED,
  errors=[PricingErrorCode.UNSUPPORTED_PRODUCT])` for every valid
  bundle.
- Tests: `tests/test_bli_zero_curve_nodes.py` (Option Discount Curve
  and Deposit Curve nodes built from the existing fixture via
  `select_curve_points_by_purpose`, matching exact expected
  year-fraction/rate pairs; sorting with reversed input;
  empty-input/non-`BLICurvePoint` rejection; every non-continuous
  `BLICurveRateBasis` member rejected; raw-string-coerced continuous
  basis working; duplicate parsed year-fraction rejection; signed-rate
  preservation; and module-boundary checks confirming no
  `pricing.curve`/`RateCurve`/`tenor_to_years`/
  `BLIMVPInputBundle`/`price_bli_mvp` import, and no
  interpolation/discount-factor/forward-price/Black-76/PV-shaped
  names). `python -m pytest -q` → 758 passed; `ruff check
  src/shiori_pricing_lab tests` → only the same 2 pre-existing,
  unrelated `products/bond_option.py` `E501` findings remain.
- **Issue #38 remains open.**
...
- **Outside the node range: flat extrapolation, rate only.** Returns
  the nearest endpoint's `zero_rate` — no fallback-flag object, no
  audit output, no warning (left for a future slice per Annex A's own
  "並標示 fallback flag" wording).
- **Single-node curves** (matching the existing Deposit Curve fixture)
  return that node's rate for any valid target.
...
- **This module does not compute discount factors, does not compute
  forward clean price, does not compute PV, and is not wired into
  `price_bli_mvp`.** It consumes only `BLIContinuousZeroCurveNode`
  values — no tenor parsing, no curve-purpose selection, no
  `BLICurvePoint`/`BLICurveRateBasis` inspection, no par-rate
  conversion. `pricing/bli_curve_tenor.py`,
...
- Tests: `tests/test_bli_zero_rate_interpolation.py` (exact node match
  ...
  order invariance; flat extrapolation both below and above range; the
  single-node Deposit Curve fixture returning its rate for three
  targets; empty-input/non-node rejection; every invalid target/node
  `year_fraction` value rejected; invalid node `zero_rate` rejected;
