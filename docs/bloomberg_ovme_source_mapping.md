...
| SWDF S490 | Stripped Curve — Zero Rate | Zero-rate nodes of the option discount curve | `curve_points[]` with `curve_purpose = OPTION_DISCOUNT_CURVE`, `rate_basis = CONTINUOUS_ZERO_RATE` | Bloomberg percent display → decimal | PARTIALLY_CONFIRMED | Preserve source date, curve side MID, and curve identity as provenance. Status stays partial until the exact zero-rate compounding/basis expected by Shiori is validated |
