...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-empty",
        action="store_true",
        help="Exit successfully when no source URL is configured.",
    )
    args = parser.parse_args()

    if not args.source_url:
        message = (
            "No CME_CTD_SOURCE_URL configured. Skipping CTD refresh. "
            "Set a repository variable/secret or pass --source-url."
        )
        print(message)
