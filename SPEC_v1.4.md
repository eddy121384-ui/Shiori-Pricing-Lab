...
|---|---|---|
| European price-based option | Black-76 on forward bond price，price state 由 `BOND_OPTION_PRICE_BASIS` 選定（`DIRTY` / `CLEAN`，預設 `DIRTY`，見 Annex A §A.2 / §A.12） | Model input 為 annualized lognormal bond price vol `σ_P`；`F`、`K`、`σ_P` 與 duration 分母必須同 basis，禁止混用。預設 `BOND_VOL_SOURCE_MODE=VCUB_NORMAL_PROXY`，依 Annex A.8 由 VCUB normal swaption vol 推導 `σ_Y^N` 再以 duration 轉為 `σ_P` |
